...
const deletePhoto = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ id: photo._id, message: "Foto excluida com sucesso." });
    } catch (error) {
        res.status(404).json({ errors: ["Foto não encontrada."] });
        return;
    }
...
const likePhoto = async (req, res) => {
    ...
    return res.status(200).json({ photoId: id, userId: reqUser._id, message: "Foto foi curtida!" });
}
...
const commentPhoto = async (req, res) => {
    ...
    return res.status(200).json({ comment: userComment, message: "Comentario adicionado com sucesso." });
}
